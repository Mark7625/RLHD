...
final class ParticleDevUi
{
	...
	static final int LEFT_WIDTH = 440;
	static final float BODY_FONT_SIZE = 13f;
	static final float SEGMENT_FONT_SIZE = 13f;
	static final float TAB_FONT_SIZE = 13f;

	private static final Border CARD_BORDER = BorderFactory.createCompoundBorder(
		BorderFactory.createLineBorder(ColorScheme.MEDIUM_GRAY_COLOR),
		BorderFactory.createEmptyBorder(8, 10, 8, 10));

	private ParticleDevUi()
	{
	}

	static void themeFrame(JComponent root)
	{
		root.setBackground(ColorScheme.DARK_GRAY_COLOR);
		root.setForeground(ColorScheme.LIGHT_GRAY_COLOR);
	}

	static void themePanel(JPanel panel)
	{
	...
	static JLabel subtitleLabel(String text)
	{
		JLabel label = new JLabel(text);
	...
	static Font bodyFont(Font base)
	{
		return base.deriveFont(BODY_FONT_SIZE);
	}

	static void styleMainTabs(javax.swing.JTabbedPane tabs)
	{
	...
	static void styleList(javax.swing.JList<?> list)
	{
	...
	static void applyListRendererFont(DefaultListCellRenderer renderer)
	{
		renderer.setFont(bodyFont(renderer.getFont()));
	}

	static void compactButton(JButton button)
	{
	...
	static void styleSegmentButton(JButton button, boolean selected)
	{
	...
	static void sizeField(JComponent field)
	{
	...
	static JComponent formRow(String label, JComponent field)
	{
		sizeField(field);
	...
	static JComponent vectorRow(String label, JComponent x, JComponent y, JComponent z)
	{
		sizeField(x);
		sizeField(y);
		sizeField(z);
	...
	static Section section(String title, boolean expanded, Runnable onToggle, JComponent... rows)
	{
