...
public class LavaTypeManager {
	...
	public void startUp() {
		fileWatcher = LAVA_TYPES_PATH.watch((path, first) -> {
			if (first) {
				loadLavaTypes(path, true);
			} else {
				clientThread.invoke(() -> loadLavaTypes(path, false));
			}
		});
	}

	private void loadLavaTypes(ResourcePath path, boolean first) {
		try {
