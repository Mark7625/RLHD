...
public class EffectorDebugOverlay extends Overlay {
	...
	public Dimension render(Graphics2D g) {
		...
		for (Placement placement : effectorDefinitionManager.getAllPlacements()) {
			...
			int[] loc = worldToLocalFirst(ctx, wp, local);
			...
			if (whirlpoolZone) {
				...
				EffectorEffect radial = findRadialEffect(def);
				drawSphereWire(g, proj, pt, baseX, baseY, baseZ, radius);
				drawRadialSpokes(g, proj, pt, baseX, baseY, baseZ, radius, radial != null && radial.type == EffectorEffect.EffectType.REPEL);
			} else if (windZone) {
	...
	private float computeDebugRadius(EffectorDefinition def) {
		...
		if (def.radiusTiles > 0f) {
			...
			EffectorEffect primary = getPrimaryEffect(def);
	...
	private void drawWindLabel(
		...
		String flow = String.format(
			"speed %.2f  intensity %.2f  units %.0f  response %.1f",
			wind.speed, wind.intensity, wind.speedUnits, wind.response
		);
		String tuning = String.format(
			"turb %.0f  gust %.0f (x%.2f)  lift %.0f  variance %.0f",
			wind.turbulence, wind.gust, wind.gustSpeed, wind.lift, wind.directionVariance
		);
	...
	@Nullable
	private EffectorEffect getPrimaryEffect(EffectorDefinition def) {
	...
	@Nullable
	private EffectorEffect findRadialEffect(EffectorDefinition def) {
		for (EffectorEffect effect : def.effects) {
			if (effect.radial) {
				return effect;
			}
	...
	@Nullable
	private static int[] worldToLocalFirst(SceneContext ctx, WorldPoint worldPoint, int[] out) {
		int[] contiguous = ctx.worldToLocal(worldPoint);
		if (contiguous != null) {
			if (out != null && out.length >= 3) {
				System.arraycopy(contiguous, 0, out, 0, 3);
				return out;
			}
			return contiguous;
		}
		return ctx.worldToLocals(worldPoint).findFirst().map(loc -> {
			if (out != null && out.length >= 3) {
				System.arraycopy(loc, 0, out, 0, 3);
				return out;
			}
			return loc;
		}).orElse(null);
	...
	private float terrainHeight(SceneContext ctx, int localX, int localY, int plane) {
		var wv = client.getTopLevelWorldView();
		LocalPoint lp = new LocalPoint(localX, localY, wv != null ? wv.getId() : -1);
